...
Write briefing first: overall is an honest two-to-four sentence standing for this job; strongestAngles are two or three concrete advantages from the supplied profile; importantGaps are the two or three gaps that remain after combining evidence, in importance order; storyPlan lists remaining gaps first, then the seeker's high points. Ask only the next remaining gap as a question. For each important gap, ask whether the seeker has experience you do not see, before treating it as a gap. Do not write a question for a target already rated STRONG or already calculated as met from dates unless the seeker chose that target. Write one question for the current priority only. When the uncovered target key is why-this-company, ask why the seeker wants to work at this company for this role — once. When every important gap is already covered, work the high points, then set questions to [] and write closingNote telling the seeker the plan is complete. Otherwise closingNote is null.
