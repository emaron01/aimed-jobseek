...
Every output claim must cite one or more supplied source ids. The quote should be the supporting words from that source. A claim may restate a source fact in different words when every number, employer, title, date, scope, and outcome still matches. Never cite a job requirement as proof that the seeker has a skill or accomplishment. A seeker claim may use only PROFILE_FACT, APPROVED_STATEMENT, or APPROVED_STORY sources.
...
Match the seeker's supplied voice samples without copying their factual content. Every fact about the seeker must cite PROFILE_FACT, APPROVED_STATEMENT, or APPROVED_STORY evidence. A quote may be the source wording; the claim may paraphrase that source when the facts stay the same. Company facts must cite COMPANY_RESEARCH when those sources are supplied. When they are not, cite only JOB_REQUIREMENT for the posted employer, title, and stated role facts, and do not invent company facts. Job outcomes may cite JOB_REQUIREMENT. Never use a job requirement as proof that the seeker has done something.
...
Audit every factual clause before returning it. Do not call a role current when its end date is null; null means unknown. Do not turn the employer's planned work into the seeker's experience. Do not characterize a system as production or customer-facing unless a seeker source says so. Do not broaden "incident response" into an unsupported domain or scope. Mention an experience gap only when an ASSESSMENT source explicitly supports that gap. A story paragraph may use two sentences so the action and the result stay together. Do not mention prompts, sources, confidence, research status, missing data, or any other internal system state. Do not add unsupported metrics, scope, titles, employers, dates, skills, credentials, or achievements. Write the way the seeker writes, in plain professional language. Use the seeker's voice samples and their own words from consultation answers whenever those are supplied. Avoid phrasing that reads as AI-generated. If qualityFeedback names a field, rewrite only that field. Return structured JSON only.`;

export const ASSET_CLAIM_VALIDATION_INSTRUCTIONS = `Audit application-material claims against their cited sources.

Decide by meaning, not by shared wording. A claim is supported when its facts — numbers, employers, titles, dates, scope, and outcomes — match a cited Personal Profile FACT or approved statement, even when the sentence is worded differently. Flag a claim when any skill, title, employer, date, credential, metric, or achievement is not supported by the cited source text. Accept a careful semantic framing that is directly entailed by the evidence: for example, reducing a failure rate and owning on-call can be described as production-reliability experience, and billing or invoice systems that the seeker shipped or rewrote may be called production systems. Do not accept a new concrete fact: for example, an API must not be called a production service unless a source says it ran in production. Flag altered numbers, inflated scope, combined facts that change meaning, and citations that do not actually support the claim. Word overlap alone is never enough to accept or reject a claim.
