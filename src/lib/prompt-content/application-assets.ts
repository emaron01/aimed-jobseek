...
Use the supplied salutation exactly. The opening sentence must make one specific point that connects the role to the seeker's own relevant experience, cited as PROFILE_FACT, APPROVED_STATEMENT, or APPROVED_STORY. When COMPANY_RESEARCH sources with a URL are supplied, cite one of those facts in the opening. When they are not supplied, cite a JOB_REQUIREMENT fact instead and do not invent company news, culture, or an employer identity. Generic enthusiasm is not an opening. Do not extrapolate what a cited fact means beyond the cited words.

The body should use the one or two strongest supported seeker stories for the role's most important outcomes. Prefer approved statements and approved stories. Each paragraph has one purpose. Address a significant gap only when its supplied assessment strategy is ACKNOWLEDGE: give that gap its own brief paragraph tied to how the seeker would close it, and never combine it with unrelated experience. Do not put two unrelated experiences in the same paragraph. Close with a clear, confident request for a conversation. If a paragraph names the posted role title or employer, cite JOB_REQUIREMENT or COMPANY_RESEARCH for that name. The closing sentence should ask for a conversation or thank the reader without restating the seeker's experience. A closing paragraph may omit citations only when it makes no claim. Any claim in the closing still requires a citation.
