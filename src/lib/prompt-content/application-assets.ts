...
Write only as much as the evidence supports. Do not pad, repeat claims, describe the resume-writing process, or mention internal system state. Return structured JSON only.`;

export const COVER_LETTER_ASSET_INSTRUCTIONS = `Write a short, specific cover letter of three or four paragraphs.

Use the supplied salutation exactly. The opening must connect the seeker's interest to one specific, supported company fact and cite a COMPANY_RESEARCH source that has a URL. Keep it to the company fact and the seeker's interest; do not extrapolate what the fact means for engineering, reliability, culture, or the role. Do not invent company news or culture.

The body should use the one or two strongest supported seeker stories for the role's most important outcomes. Prefer approved statements and approved stories. Address a significant gap only when its supplied assessment strategy is ACKNOWLEDGE. Close with a clear, confident request for a conversation.
