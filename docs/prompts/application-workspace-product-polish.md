...
TASK: Restructure the application workspace from one long page into an overview plus one page per step, with a numbered progress tracker in the side navigation, Harper as a docked side panel on every step, a visual design system, and a product polish pass. Work on main. Commit and push when all checks pass. If a progress tracker already exists from an earlier task, build on it rather than redoing it.
WHY
The workspace is one long page of twelve stacked sections. The seeker scrolls to find things, everything competes for attention, nothing shows where they are, and ready notices stack up (production showed "Cover Letter is ready. View it" four times). Well-designed workflow products use a step rail with one step per page, an overview per item, and an assistant docked beside the work. Harper is the seeker's coach and belongs beside every step, not in a section they scroll to. The product must look and read like one designed product, not features added one at a time.
PART 1: Structure
...
3. Status pills, cards, section headers, empty states, and skeleton loaders, all from shared components.
...
1. Copy: review every seeker-facing string. Plain, warm, direct language in one consistent voice; sentence case for headings and buttons; the same verb for the same action everywhere (for example, always "Generate", never a mix of "Create", "Build", and "Make" for the same kind of action). No system or developer language anywhere: no job, queue, validation, schema, enum, or id wording, and no raw error text.
2. Consistency: one page layout with a consistent page header (title, one-line description, primary action on the right), one spacing scale, and one card style across every page.
3. States: every view has a designed loading state (skeleton), empty state (guidance plus the next action), and error state (plain explanation plus a retry). No blank areas and no unstyled messages.
4. Icons: one icon set used consistently for steps, statuses, and actions.
5. Motion: subtle transitions for opening panels, expanding sections, and status changes, respecting the reduced-motion setting.
6. Leftovers: remove any debug text, raw data, internal labels, or developer-facing controls from seeker-facing pages. Page titles in the browser tab name the page and application.
7. Review pass: after the build, screenshot every seeker-facing page, check each against this list, fix what fails, and include the before-and-after screenshots in the report.
TESTS
...
- No seeker-facing string contains internal terms (job, queue, validation, schema, enum, raw ids).
- Every view renders designed loading, empty, and error states.
VERIFY
...
REPORT
The route structure, the step definitions of done, the token set with its color values and contrast results, before-and-after screenshots of every seeker-facing page (including the overview, two step pages with Harper docked, and the small-screen tracker), copy changes by area, files changed, and a full-suite result.
