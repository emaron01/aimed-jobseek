...
- Company research batches are rows in `ResearchRun` (list-scoped via `contactListId`, or application employer research via `campaignId`). Application save actions enqueue a `PENDING` run and return immediately; they do not call `researchCompany` in the web request. `scripts/research-worker.ts` polls every 5 seconds, claims a run, and calls `processResearchRun` in `src/lib/research/runs-service.ts`. The web service does not process the queue. On Render this is a separate Background Worker (`npm run research:worker:render`). The web service owns migrations via `scripts/render-pre-deploy.mjs`. The worker waits until the `ResearchRun` table exists and does not migrate. There is no `render.yaml` in the repo; create the worker in the Render dashboard.
